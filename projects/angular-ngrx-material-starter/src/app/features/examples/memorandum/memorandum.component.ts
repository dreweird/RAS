...
export class MemorandumComponent implements OnInit {
  ...
  rowMemorandum(){
    this.recordService.getAllDocumentType('Memorandum').subscribe(data => {
      console.log(data);
      this.rowData = data;
  ...
  openDialog() {
    this.dialog.open(EntryDialogComponent, {
      minWidth: '50vh',
      data: { gridApi: this.gridApi, add: true, type:  'Memorandum'}
    });
  }
  onGridReady(params) {
  ...
  methodFromParentDelete(cell) {
    this.dialog.open(EntryDialogComponent, {
      minWidth: '50vh',
      data: { gridApi: this.gridApi, data: cell, delete: true,  type:  'Memorandum' }
    });
  }
  ...
  methodFromParentEdit(cell) {
    this.dialog.open(EntryDialogComponent, {
      minWidth: '50vh',
      data: { gridApi: this.gridApi, data: cell, edit: true, type:  'Memorandum' }
    });
  }
