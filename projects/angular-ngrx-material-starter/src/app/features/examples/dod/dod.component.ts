...
export class DodComponent implements OnInit {
  ...
  rowMemorandum(){
    this.recordService.getAllDocumentType('DOD').subscribe(data => {
      console.log(data);
      this.rowData = data;
  ...
  openDialog() {
    this.dialog.open(EntryDialogComponent, {
      minWidth: '50vh',
      data: { gridApi: this.gridApi, add: true, type:  'DOD'}
    });
  }
  onGridReady(params) {
  ...
  methodFromParentDelete(cell) {
    this.dialog.open(EntryDialogComponent, {
      minWidth: '50vh',
      data: { gridApi: this.gridApi, data: cell, delete: true,  type:  'DOD' }
    });
  }
  ...
  methodFromParentEdit(cell) {
    this.dialog.open(EntryDialogComponent, {
      minWidth: '50vh',
      data: { gridApi: this.gridApi, data: cell, edit: true, type:  'DOD' }
    });
  }
